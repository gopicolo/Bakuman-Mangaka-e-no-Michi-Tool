...
POINTER_ADJUSTMENT = 45  # The magic value 0x2D we discovered
...
def extract_from_file(filepath):
    ...
    try:
        ...
        for i in range(4, header_size, 5):
            if i + 5 > header_size: break
            tag = content[i]
            pointer_base = int.from_bytes(content[i+1:i+5], 'little')
            pointers.append({
                "offset_in_header": i,
                "tag": f"{tag:02X}",
                "pointer_base_hex": content[i+1:i+5].hex(' '),
                "original_target": pointer_base + POINTER_ADJUSTMENT
            })
    ...
    while current_pos < len(content):
        ...
        if not is_choice_block:
            ...
            while True:
                ...
                if len_offset >= len(content): break
                ...
                if length > 0 and payload_end <= len(content):
                    ...
                    strings.append({"type": "dialogue", "length_offset": len_offset, "original_length": length, "original_text": robust_decode(payload)})
                chain_pos = payload_end
                if not (chain_pos + 1 < len(content) and content[chain_pos] == 0x1F): break
                chain_pos += 1
            ...
            for i in range(num_options):
                if choice_pos + 2 > len(content): break
                ...
                if length > 0 and payload_end <= len(content):
                    payload = content[payload_start:payload_end]
                    strings.append({
                        "type": "choice", "tag": f"{tag_val:02X}",
                        "length_offset": len_byte_offset, "original_length": length,
    ...
    for i, entry in enumerate(strings):
        ordered_entry = {
            "id": i, "type": entry.get("type", "dialogue"), "tag": entry.get("tag"),
            "length_offset": entry['length_offset'], "original_length": entry['original_length'],
            "original_text": entry['original_text'], "translated_text": ""
        }
        final_data.append(ordered_entry)

    for ptr in pointers:
        ptr['original_target'] = f"0x{ptr['original_target']:X}"

    print(f"  {len(final_data)} strings and {len(pointers)} pointers found.")
    return {"pointers": pointers, "strings": final_data}
...
def main():
    ...
    unextracted_files = []
    print(f"--- Starting Final Script .scr Extractor ---")
    ...
    if unextracted_files:
        print(f"\n--- {len(unextracted_files)} .scr files with no data extracted ---")
        for filename in sorted(unextracted_files):
            print(f"- {filename}")
    else:
        print("\n--- Final Report ---")
        print("All .scr files were successfully extracted!")
